...
5. **Casing: all caps is mono-only** — **`View.jotUI(_:)`** + **`FontManager.uiLabel2`…`uiMicro`** / **`uiHeadingH4`** / **`uiPro`** render **SF Pro**. Use **natural casing** (typically **sentence case**; Title Case only when product copy dictates). **Do not** add **`.textCase(.uppercase)`** to proportional UI labels to “match Figma.” **All caps is reserved exclusively** for text set in **`FontManager.metadata`** (SF Mono) via **`jotMetadataLabelTypography()`** — see item 6.

6. **Monospace metadata + all caps** — For static **`Text`** in the **monospaced** technical face only: **`jotMetadataLabelTypography()`** (see `.cursor/rules/metadata_label_typography.mdc`) — **11pt**, **medium**, **all caps**. **Do not** apply **negative** tracking to mono all-caps chrome. Apple recommends **opening** uppercase slightly at small sizes; if an overlay is **exceptionally dense** (e.g. global search footer), a **small extra positive** `.tracking` may be added **in that component** with an inline comment pointing to this section — not by copying Figma’s negative values.
...
| `uiPro` / `uiHeadingH4` / `uiLabel2`–`uiMicro` | SF Pro                                                    | Figma scale (20…9)               | Per style          | App chrome: **`jotUI(…)`** + **`proportionalUITracking`**; **sentence case** (no forced all caps); see `FontManager.UITextRamp` |

**SF Pro UI labels (`jotUI` / `uiLabel*`):** Non-user **`Text`** in the proportional chrome ramp uses **normal casing** — not **`.textCase(.uppercase)`** unless product explicitly documents an exception.

**Monospaced static labels only (`jotMetadataLabelTypography`):** For **`Text`** in **`FontManager.metadata`** / SF Mono — **11pt, medium, all caps** via `jotMetadataLabelTypography()` (or manual `.font(FontManager.metadata(…))` + `.textCase(.uppercase)`). Do not ship sentence-case **mono** labels except where product explicitly overrides. **Never** force all caps on **`TextField`** / **`TextEditor`** (use `metadata` font **without** `.textCase(.uppercase)` on typed fields).
