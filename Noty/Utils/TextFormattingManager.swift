...
import SwiftUI
    ...
    // iOS implementation using UITextView
    @MainActor
    class TextFormattingManager: ObservableObject {
        @Published var isBold = false
        @Published var isItalic = false
        ...
        func applyFormatting(to textView: UITextView, tool: EditTool) {
            guard let textStorage = textView.textStorage else { return }
            ...
            case .lineBreak:
                textView.insertText("\n")
            default:
                // Other tools can be implemented as needed
                break
            }
        }
        ...
        private func insertDivider(to textView: UITextView) {
            guard let textStorage = textView.textStorage else { return }

            let selectedRange = textView.selectedRange

            // Get the available width from the text container (adaptive to editor width)
            let availableWidth = textView.textContainer.size.width
            let lineWidth = availableWidth - 20  // Subtract padding for margins
            ...
            let image = renderer.image { context in
                UIColor.label.withAlphaComponent(0.3).setFill()
