...
/// Comprehensive data backup and persistence management for Jot app
@MainActor
final class DataBackupManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.jot.app", category: "DataBackup")
    ...
    private var backupDirectory: URL {
        ...
        let jotSupport = appSupport.appendingPathComponent("Jot")
        let backupsDir = jotSupport.appendingPathComponent("Backups")
    ...
    private var swiftDataStoreURL: URL? {
        let appSupport = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let jotSupport = appSupport.appendingPathComponent("Jot")
        let storeURL = jotSupport.appendingPathComponent("default.store")
    ...
    func getAvailableBackups() -> [BackupMetadata] {
        do {
            ...
            return backupFiles
                .filter { $0.pathExtension == "jotbackup" }
...
private extension DataBackupManager {
    ...
    func _performBackup() async -> BackupResult {
        ...
        do {
            ...
            let backupFileName = "jot_backup_\(timestamp).jotbackup"
