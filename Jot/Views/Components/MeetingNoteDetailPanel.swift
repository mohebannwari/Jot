...
private struct SessionAccordion: View {
  ...
  @Namespace private var sessionTabPillNamespace
  ...
  init(
    session: MeetingSession, defaultExpanded: Bool = false,
    onNotesChanged: ((String) -> Void)? = nil, onSummaryChanged: ((String) -> Void)? = nil
  ) {
  ...
  private var sessionTabPillSlideAnimation: Animation {
  ...
  private var tabBar: some View {
    HStack(spacing: 2) {
      ForEach(MeetingTab.allCases) { tab in
        Button {
          withAnimation(sessionTabPillSlideAnimation) {
            selectedTab = tab
          }
        } label: {
          ...
          Text(tab.label)
            ...
            .background {
              if selectedTab == tab {
                Capsule()
                  .fill(tabSelectedBackground)
                  .matchedGeometryEffect(
                    id: "meetingDetailSessionTabPill", in: sessionTabPillNamespace)
              }
            }
        }
        .buttonStyle(.plain)
