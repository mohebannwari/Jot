...
struct FloatingSearch: View {
  ...
  /// (`TabsContainerOverlayView.blocksColor` / `FloatingEditToolbar` picker pill fill / Figma `bg/blocks`).
  ...
  var body: some View {
    ...
    .onChange(of: appleIntelligenceService.meetingNotesCapability.showsEntryPoints) {
      _, isEnabled in
    ...
    .onReceive(
      NotificationCenter.default.publisher(
        for: AppCommand.Kind.floatingSearchSwitchToMeetingPickNote.name)
    ) { _ in
  ...
  private func typedFilterQuickActionRow(spec: RootQuickActionFilterSpec, unifiedIndex: Int)
    -> some View
  {
  ...
  private func resultRow(_ result: SearchHit, unifiedIndex: Int) -> some View {
    ...
    .onHover { hovering in
      withAnimation(SearchAnimations.resultHover) {
        hoveredResultID =
          hovering ? result.id : (hoveredResultID == result.id ? nil : hoveredResultID)
        if hovering {
          selectedResultIndex = unifiedIndex
        }
      }
  ...
  private func selectCurrentResult(recordQuery: Bool = true) {
    guard !engine.results.isEmpty,
      selectedResultIndex < engine.results.count
    else { return }
...
extension View {
  ...
  fileprivate func floatingSearchKeycapShadows() -> some View {
    self
  ...
  private final class FloatingSearchTextFieldCell: NSTextFieldCell {
    ...
    override func edit(
      ...
      super.edit(
        withFrame: drawingRect(forBounds: rect), in: controlView, editor: textObj,
