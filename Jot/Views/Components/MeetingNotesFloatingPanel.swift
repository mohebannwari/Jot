...
struct MeetingNotesFloatingPanel: View {
  ...
  @Namespace private var meetingTabPillNamespace
  ...
  /// Easing for the active-tab capsule when it slides between Summary / Transcript / Notes.
  private var meetingTabPillSlideAnimation: Animation {
    accessibilityReduceMotion ? .easeInOut(duration: 0.12) : .easeInOut(duration: 0.28)
  }
  ...
  /// Selected tab pill, recording-row square actions, and post-recording **Dismiss** capsule: neutral-800 in dark (`SecondaryBackgroundColor`); light keeps accent-tinted secondary.
  private var meetingPanelSubchromeSurfaceFill: Color {
    colorScheme == .dark
      ? Color("SecondaryBackgroundColor")
      : themeManager.tintedSecondaryButtonBackground(for: colorScheme)
  }
  ...
  private var microPillChrome: some View {
    ...
    .modifier(
      MeetingPanelBackgroundModifier(
  ...
  private var tabBar: some View {
    HStack(spacing: 4) {
      ForEach(visibleTabs) { tab in
        Button {
          withAnimation(meetingTabPillSlideAnimation) {
            selectedTab = tab
          }
        } label: {
          // Selection signal: capsule fill + foregroundColor delta (weight is invariant; chrome never uses weight for state).
          Text(tab.label)
            .jotUI(FontManager.uiLabel5(weight: .regular))
            .foregroundColor(
              selectedTab == tab ? Color("PrimaryTextColor") : Color("SecondaryTextColor")
            )
            ...
            .background {
              if selectedTab == tab {
                Capsule()
                  .fill(meetingPanelSubchromeSurfaceFill)
                  .matchedGeometryEffect(id: "meetingFloatingTabPill", in: meetingTabPillNamespace)
              }
            }
        }
        .buttonStyle(.plain)
  ...
  private var completeControls: some View {
    HStack(spacing: 4) {
      ...
      .buttonStyle(.plain)
      .background(meetingPanelSubchromeSurfaceFill, in: Capsule())
      .macPointingHandCursor()
      .subtleHoverScale(1.04)
    }
  }
