...
struct PillOffsetKey: PreferenceKey {
  ...
  static func reduce(
    value: inout [ToolbarSubmenuType: CGFloat], nextValue: () -> [ToolbarSubmenuType: CGFloat]
  ) {
...
struct FloatingEditToolbar: View {
  ...
  /// Picker capsule fill: light keeps `ToolbarPillColor`; dark uses neutral-800
  /// (`SecondaryBackgroundColor` per design tokens) for the four segmented pills.
  private var pickerPillFill: Color {
    colorScheme == .dark ? Color("SecondaryBackgroundColor") : Color("ToolbarPillColor")
  }
  ...
  /// Picker capsule chrome; subtle shadow only in light mode (none in dark).
  @ViewBuilder
    ...
    .background(
      GeometryReader { geo in
        Color.clear.preference(
          key: PillOffsetKey.self, value: [.textOptions: geo.frame(in: .named("toolbar")).midX])
      })
  }
  ...
  private var fontSizePill: some View {
    ...
    .background(
      GeometryReader { geo in
        Color.clear.preference(
          key: PillOffsetKey.self, value: [.fontSize: geo.frame(in: .named("toolbar")).midX])
      })
  }
  ...
  private var fontPickerPill: some View {
    ...
    .background(
      GeometryReader { geo in
        Color.clear.preference(
          key: PillOffsetKey.self, value: [.fontFamily: geo.frame(in: .named("toolbar")).midX])
      })
  }
  ...
  private var colorTriggerPill: some View {
    ...
    .background(
      GeometryReader { geo in
        Color.clear.preference(
          key: PillOffsetKey.self, value: [.color: geo.frame(in: .named("toolbar")).midX])
      })
  }
  ...
  private func submenuIconButton(_ assetName: String, submenu: ToolbarSubmenuType) -> some View {
    Button {
      ...
      Image(assetName)
        ...
        .frame(width: 15, height: 15)
        .foregroundColor(
          activeSubmenu == submenu ? Color("AccentColor") : Color("IconSecondaryColor"))
    }
    .buttonStyle(.plain)
    .frame(width: 28, height: 28)
    .contentShape(Rectangle())
    .background(
      GeometryReader { geo in
        Color.clear.preference(
          key: PillOffsetKey.self, value: [submenu: geo.frame(in: .named("toolbar")).midX])
      })
  }

  private func toolIconButton(_ assetName: String, tool: EditTool, isActive: Bool = false)
    -> some View
  {
    Button {
      HapticManager.shared.toolbarAction()
