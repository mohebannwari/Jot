...
## Agent Architecture & Context Efficiency

**The primary law: the main context window is sacred. Keep it clean and focused.**

- **Strategic Orchestration:** As the lead Gemini CLI agent, delegate high-volume or speculative tasks to sub-agents (`generalist`, `codebase_investigator`) to preserve the main session history. Keep your own context lean.
- **Parallel Execution:** Always parallelize independent tool calls (e.g., `grep_search`, `read_file`, `mcp_context7_query-docs`) to gather information quickly without wasting turns.
- **Surgical Edits:** Use precise `start_line` and `end_line` for `read_file`, and precise `replace` commands to minimize context usage when reading or editing code.
- **Fire-and-forget:** Never block waiting for trivial information. Use Context7 MCP for SDK docs, Figma MCP for tokens, and fast `grep_search`/`glob` for codebase pattern discovery.
...
4. Utilize `enter_plan_mode` for designing complex changes securely.

---

## Tool Calling & MCP Priority

**Always invoke the right tools/skills before acting:**

- **Documentation & APIs:** Use `mcp_context7_resolve-library-id` followed by `mcp_context7_query-docs` (Always first for SwiftUI/SDK docs).
- **Design & UI:** Utilize `mcp_figma_...` tools to get context straight from Figma.
- **Deep Investigation:** Use `codebase_investigator` sub-agent for complex root-cause analysis or architectural mapping.
- **Web Search:** `google_web_search` or `web_fetch` as a last resort if Context7 lacks the specific answer.
...
**NEVER launch a new app window.** The user already has Jot running. Capture the existing window — do not use `open` on the built `.app` bundle. To screenshot the already-running window via `run_shell_command`:
...
- Always use Context7 MCP (`mcp_context7_query-docs`) for SwiftUI/SDK documentation.
